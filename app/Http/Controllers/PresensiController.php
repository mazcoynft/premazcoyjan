    ...
    public function cetakrekap(Request $request)
{
    ...
    $i = 1;
    $dari = $tanggal_mulai;
    while (strtotime($dari) <= strtotime($tanggal_akhir)) {
        $rangetanggal[] = $dari;
    ...
    $query->leftJoin(
        DB::raw("(
            ...
            LEFT JOIN  jam_kerja ON presensi.kode_jam_kerja = jam_kerja.kode_jam_kerja
            LEFT JOIN pengajuan_izin ON presensi.kode_izin = pengajuan_izin.kode_izin
            WHERE tgl_presensi BETWEEN '$tanggal_mulai' AND '$tanggal_akhir'
            GROUP BY nik
        ) presensi"),
        function ($join) {
    ...
    return view('presensi.cetakrekap', compact('tanggal_mulai', 'tanggal_akhir', 'bulan', 'tahun', 'namabulan', 'rekap', 'rangetanggal', 'jmlhari'));
}
